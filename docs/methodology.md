...
The output of the above step is `panic_survey.json`, which lists 350 panicking
...
The breakdown of the 350 panic sites:

The 350 is a count of panic *instructions* (`bl`s); this table reconciles it down to the
...
|---|--:|---|
| Panic instructions (`bl`s in the release binary) | **350** | one record per `bl` in `panic_survey.json` |
| − monomorphization duplicates | −89 | generic code is emitted per capsule, so many `bl`s share one source line — e.g. `Grant::enter`'s re-entrancy panic (`kernel/src/grant.rs:1442`) is emitted as **50** instructions |
| **= distinct source panic sites** | **261** | unique `effective_frame` `file:line` |
| ↳ precise Tock sites | 226 | a `file:line` we can annotate directly |
| ↳ stdlib-helper sites | 13 | panic inside an inlined stdlib helper — no Tock line; the obligation belongs to the inlined caller |
| ↳ function-level only | 22 | exact line lost to LTO → annotated on the enclosing fn (`FLUX-TODO-FN-LEVEL`) |
| **live `flux_support::assert` obligations** | **~189** | one precondition authored per precise site that admits one; the remainder are explicit `panic!`/`unreachable!`/hardware-trust panics not expressible as a precondition. Grep-derived (see Phase-1 breadcrumb commands), post-dedup 2026-05-25; the Phase-2 census below was measured at 200 before that dedup |

Reviewer checks: `89 + 261 = 350`, and `226 + 13 + 22 = 261`. The biggest dedup fan-ins are
`grant.rs:1442` (50 instructions → 1 line), `leasable_buffer.rs:372` (8),
`optional_cell.rs:202` (5), `take_cell.rs:127` (4).

The flavor / crate cuts below are at the **instruction** level (they sum to 350).
...
the whole set is greppable. **These are per-panic-site status markers** (currently keyed to the
binary `addr=`, which ties each back to its `panic_survey.json` record) — *not* the obligations;
their counts track panics, and a *different marker is used per situation*. The counts below are
**reproducible** — they drift as work proceeds, so re-run the command rather than trust a frozen
number:
...
# per marker (counts shown are as of 2026-05-25)
grep -rn 'FLUX-TODO '         --include='*.rs' kernel libraries arch capsules chips | wc -l   # 243
grep -rn 'FLUX-OPT '          --include='*.rs' kernel libraries arch capsules chips | wc -l   #  19
grep -rn 'FLUX-TODO-FN-LEVEL' --include='*.rs' kernel libraries arch capsules chips | wc -l   #  19
grep -rn 'FLUX-TODO-BLOCKED'  --include='*.rs' kernel libraries arch capsules chips | wc -l   #  10

# live (uncommented) obligations
grep -rn 'flux_support::assert(' --include='*.rs' kernel libraries arch capsules chips \
    | grep -v '//.*flux_support::assert' | wc -l                                              # 189
```

| marker | situation |
|---|---|
| `// FLUX-TODO addr=… line=… flavor=…` | a panic flagged for verification, not yet proven-and-optimizable — most carry an authored `flux_support::assert`; some do not yet (`reason=…` records the blocker when one can't be authored) |
| `// FLUX-OPT addr=… line=… flavor=…` | precondition is in place and goes through; the runtime `assert!` can become `assert_unchecked` / `get_unchecked` (provable **and** removable from the binary) |
| `// FLUX-TODO-FN-LEVEL covers=[addr…] flavor=…` | exact line lost to LTO — one marker on the enclosing fn, covering the listed addresses |
| `// FLUX-TODO-BLOCKED …` | cannot be proven yet — blocked on a cell invariant, `dyn`-in-sig ICE, re-entrancy, etc. |

**Markers ≠ obligations** (why ~291 markers but ~189 obligations): these comments count
*panics*, not authored preconditions, and the two diverge by design — many of the 243
...
line to mark at all (so the marker counts also don't sum to 350). The *reason* a `BLOCKED`
...
**The `addr=` key is unstable (migration pending).** The breadcrumb's `addr=` is a
release-binary instruction address, which drifts on every LTO rebuild. Re-running the annotator
against a drifted binary re-resolves addresses and can emit a *second* marker for a site with a
different `flavor` guess, so one source panic accumulates duplicate/conflicting markers. A
2026-05-25 sweep found 20 double-marked addresses: 5 were stale orphans (removed), 5 are **not**
duplicates (2 intentional multi-site markers — `reason=monomorph-at-caller`,
`reason=multi-fn-ambiguous-entry`; 3 one address legitimately guarding two distinct
obligations), and 10 remain conflicting-flavor or `FLUX-TODO`/`FLUX-OPT`-superseded, pending
re-anchoring. The planned fix is to migrate the stable join key from `addr=` to
`file:line+flavor` (see the worklist in `docs/transitive_obligations_plan.md`).
...
commented-out and auxiliary asserts, the live obligation set is ~189
