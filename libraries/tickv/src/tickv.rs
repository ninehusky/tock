...
impl<'a, C: FlashController<S>, const S: usize> TicKV<'a, C, S> {
    ...
    pub fn initialise(&self, hashed_main_key: u64) -> Result<SuccessCode, ErrorCode> {
        ...
        let key_ret = match self.state.get() {
            ...
            _ => {
                ...
                // Notes: blocked-cell
                // FLUX-TODO addr=0x17e04 flavor=explicit_panic
    ...
    // FLUX-TODO-FN-LEVEL addrs=[0x162f0] flavor=assert
    fn get_region(&self, hash: u64) -> usize {
        ...
        // FLUX-TODO addr=0x162d8 flavor=rem_by_zero
    ...
    pub fn append_key(&self, hash: u64, value: &[u8]) -> Result<SuccessCode, ErrorCode> {
        ...
        loop {
            let new_region = match self.state.get() {
                ...
                // FLUX-TODO addr=0x16bb4 flavor=explicit_panic
            ...
            // Notes: blocked-cell
            // FLUX-TODO addr=0x16baa flavor=unwrap_option
    ...
    pub fn get_key(&self, hash: u64, buf: &mut [u8]) -> Result<(SuccessCode, usize), ErrorCode> {
        ...
        loop {
            ...
            let new_region = match self.state.get() {
                ...
                _ => {
                    // Notes: blocked-cell
                    // FLUX-TODO addr=0x16e8a flavor=explicit_panic
                    flux_support::assert(false);
                    unreachable!()
                },
            };
            ...
            // Notes: blocked-cell
            // FLUX-TODO addr=0x16e80 flavor=unwrap_option
            flux_support::assert(region_buf_opt.is_some());
            let region_data = region_buf_opt.unwrap();
    ...
    pub fn invalidate_key(&self, hash: u64) -> Result<SuccessCode, ErrorCode> {
        ...
        loop {
            ...
            let new_region = match self.state.get() {
                ...
                _ => {
                    // Notes: blocked-cell
                    // FLUX-TODO addr=0x16298 flavor=explicit_panic
                    flux_support::assert(false);
                    unreachable!()
                },
            };
            ...
            let region_buf_opt = self.read_buffer.take();
            // Notes: blocked-cell
            // FLUX-TODO addr=0x1628e flavor=unwrap_option
            flux_support::assert(region_buf_opt.is_some());
            let region_data = region_buf_opt.unwrap();
    ...
    pub fn zeroise_key(&self, hash: u64) -> Result<SuccessCode, ErrorCode> {
        ...
        loop {
            ...
            let new_region = match self.state.get() {
                ...
                _ => {
                    // Notes: blocked-cell
                    // FLUX-TODO addr=0x18234 flavor=explicit_panic
                    flux_support::assert(false);
                    unreachable!()
                },
            };
            ...
            // FLUX-TODO addr=0x1822a flavor=unwrap_option
            flux_support::assert(false);
            let region_buf_opt = self.read_buffer.take();
            let region_data = region_buf_opt.unwrap();
    ...
    fn garbage_collect_region(
        ...
        // Notes: blocked-cell
        // FLUX-TODO addr=0x17f64 flavor=unwrap_option
        flux_support::assert(region_buf_opt.is_some());
        let region_data = region_buf_opt.unwrap();
    ...
    pub fn garbage_collect(&self) -> Result<usize, ErrorCode> {
        ...
        let start = match self.state.get() {
            ...
            _ => {
                // Notes: blocked-cell
                // FLUX-TODO addr=0x17f6e flavor=explicit_panic
                flux_support::assert(false);
                unreachable!()
            },
        };
