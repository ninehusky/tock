...
macro_rules! internal_err {
    [ $( $arg:expr ),+ ] => {{
        // FLUX-TODO addrs=[0x116e6, 0x117fc] flavor=explicit_panic
...
impl EndpointState {
    ...
    fn ctrl_state(self) -> CtrlState {
        match self {
            ...
            // FLUX-TODO addr=0x12514 flavor=explicit_panic
    ...
    fn bulk_state(self) -> (TransferType, Option<BulkInState>, Option<BulkOutState>) {
        match self {
            ...
            // FLUX-TODO addr=0x11560 flavor=explicit_panic
...
impl<'a> Usbd<'a> {
    ...
    fn start_dma_in(&self, endpoint: usize, size: usize) {
        // FLUX-TODO addr=0x115a8 flavor=optional_cell_unwrap
        ...
        // FLUX-TODO addr=0x11616 flavor=slice_end
    ...
    fn start_dma_out(&self, endpoint: usize) {
        // FLUX-TODO addr=0x12578 flavor=optional_cell_unwrap
    ...
    fn debug_in_packet(&self, size: usize, endpoint: usize) {
        // FLUX-TODO addr=0x115b6 flavor=optional_cell_unwrap
        flux_support::assert(self.descriptors[endpoint].slice_in.is_some());
        let slice = self.descriptors[endpoint].slice_in.unwrap_or_panic(); // Unwrap fail = No IN slice set for this descriptor
        if size > slice.len() {
            // FLUX-TODO addr=0x11638 flavor=explicit_panic
            flux_support::assert(false);
...
fn inter_endepin(ep: usize) -> Field<u32, Interrupt::Register> {
    match ep {
        ...
        _ => {
            // FLUX-TODO addr=0x124ea flavor=explicit_panic
            flux_support::assert(false);
            unreachable!()
        },
    }
...
fn packet_to_hex(packet: &[VolatileCell<u8>], packet_hex: &mut [u8]) {
    ...
    while i < packet.len() {
        let x = packet[i].get();
        // FLUX-TODO addr=0x116b4 flavor=bounds
