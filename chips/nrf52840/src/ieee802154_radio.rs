...
impl<'a> Radio<'a> {
    ...
    fn rx(&self) {
        ...
        // FLUX-TODO addr=0x12aee flavor=unwrap_option
        ...
        match self.state.get() {
            ...
            RadioState::RX => {
                ...
                if self.registers.event_end.is_set(Event::READY) {
                    ...
                    // replacing).
                    // FLUX-TODO addr=0x12af4 flavor=unwrap_option
                    flux_support::assert(self.rx_buf.is_some());
                    let rbuf = self.rx_buf.take().unwrap();
                        ...
                        self.ack_buf
                            ...
                            .unwrap_or_else(|err| {
                                ...
                                self.rx_client.map(|client| {
                                    start_task = true;
                                    // FLUX-TODO addr=0x12b12 flavor=unwrap_option
                                    flux_support::assert(self.rx_buf.is_some());
                                    client.receive(
            ...
            RadioState::TX => {
                ...
                if self.registers.event_ccabusy.is_set(Event::READY) {
                    ...
                    if self.cca_count.get() < IEEE802154_MAX_POLLING_ATTEMPTS {
                        ...
                        // FLUX-TODO addr=0x128be flavor=optional_cell_unwrap
                        flux_support::assert(self.timer0.is_some());
                        let current_time = self.timer0.unwrap_or_panic().now();
                        // FLUX-TODO addr=0x128d2 flavor=optional_cell_unwrap
                        flux_support::assert(self.timer0.is_some());
                        self.timer0
                        ...
                        self.tx_client.map(|client| {
                            ...
                            // set_dma_ptr(...)
                            // FLUX-TODO addr=0x12b0c flavor=unwrap_option
                ...
                if self.registers.event_end.is_set(Event::READY) {
                    ...
                    self.tx_client.map(|client| {
                        ...
                        // FLUX-TODO addr=0x12b00 flavor=unwrap_option
                        flux_support::assert(self.tx_buf.is_some());
                        let tbuf = self.tx_buf.take().unwrap();
                        client.send_done(tbuf, false, result);
                    });
            ...
            RadioState::ACK => {
                ...
                if self.registers.event_end.is_set(Event::READY) {
                    ...
                    // FLUX-TODO addr=0x12afa flavor=unwrap_option
                    flux_support::assert(self.tx_buf.is_some());
                    let tbuf = self.tx_buf.take().unwrap();
                    ...
                    self.rx_client.map(|client| {
                        ...
                        // FLUX-TODO addr=0x12b06 flavor=unwrap_option
                        flux_support::assert(self.rx_buf.is_some());
                        let rbuf = self.rx_buf.take().unwrap();
