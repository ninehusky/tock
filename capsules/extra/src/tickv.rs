    ...
    for TickFSFlashCtrl<'a, F>
{
    fn read_region(
        ...
        // NOTES: blocked-cell
        // FLUX-TODO addr=0x16544 flavor=unwrap_option
...
impl<'a, F: Flash, H: Hasher<'a, 8>, const PAGE_SIZE: usize> TicKVSystem<'a, F, H, PAGE_SIZE> {
    ...
    fn complete_init(&self) {
        ...
        match self.next_operation.get() {
            ...
            Operation::GetKey => {
                // FLUX-TODO addr=0x18da6 flavor=unwrap_option
            ...
            Operation::AppendKey => {
                // FLUX-TODO addr=0x18da0 flavor=unwrap_option
                flux_support::assert(self.value_buffer.is_some());
    ...
    for TicKVSystem<'a, F, H, PAGE_SIZE>
{
    fn add_mut_data_done(&self, _result: Result<(), ErrorCode>, data: SubSliceMut<'static, u8>) {
        ...
        // NOTES: blocked-cell
        // FLUX-TODO addr=0x1cfd4 flavor=unwrap_option
        flux_support::assert(self.key_buffer.is_some());
        ...
        match self.operation.get() {
                ...
                match ret {
                    ...
                    Err(tickv::error_codes::ErrorCode::BufferTooSmall(_)) => {
                        ...
                        self.client.map(|cb| {
                            ...
                            // FLUX-TODO addr=0x1d2e4 flavor=unwrap_option
                            flux_support::assert(self.key_buffer.is_some());
                    ...
                    Err(e) => {
                        ...
                        self.operation.set(Operation::None);
                        self.client.map(|cb| {
                            // FLUX-TODO addr=0x1d2ea flavor=unwrap_option
                            flux_support::assert(self.key_buffer.is_some());
            ...
            Operation::AppendKey => {
                match ret {
                    ...
                    Err(e) => {
                        ...
                        self.client.map(|cb| {
                            // FLUX-TODO addr=0x1d2f6 flavor=unwrap_option
                            flux_support::assert(self.key_buffer.is_some());
            ...
            Operation::InvalidateKey => match ret {
                ...
                Err(e) => {
                    ...
                    self.client.map(|cb| {
                        // FLUX-TODO addr=0x1d2f0 flavor=unwrap_option
                        flux_support::assert(self.key_buffer.is_some());
    ...
    fn write_complete(&self, pagebuffer: &'static mut F::Page, _result: Result<(), flash::Error>) {
        ...
        match self.operation.get() {
            ...
            Operation::AppendKey => {
                self.operation.set(Operation::None);
                self.client.map(|cb| {
                    // FLUX-TODO addr=0x1d3f0 flavor=unwrap_option
                    flux_support::assert(self.key_buffer.is_some());
                    // FLUX-TODO addr=0x1d3fc flavor=unwrap_option
                    flux_support::assert(self.value_buffer.is_some());
                    cb.append_key_complete(
                        Ok(()),
                        self.key_buffer.take().unwrap_or_else(|| unsafe { core::hint::unreachable_unchecked() }),
