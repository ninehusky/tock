    ...
    pub fn encode(&self, buf: &mut [u8], offset: usize) -> SResult<usize> {
        let (offset, _) = match self.header {
            ...
            TransportHeader::UDP(udp_header) => {
                ...
                // FLUX-TODO addr=0xdb04 flavor=unwrap_option
                flux_support::assert(done.is_some());
                done.unwrap()
            }
            TransportHeader::ICMP(icmp_header) => {
                let done = icmp_header.encode(buf, offset).done();
                // FLUX-TODO addr=0xdb0a flavor=unwrap_option
                flux_support::assert(done.is_some());
                done.unwrap()
            }
            _ => {
                // FLUX-TODO addr=0xdafe flavor=explicit_panic
        ...
        // FLUX-OPT addr=0xdaf4 flavor=slice_end
...
impl<'a> IP6Packet<'a> {
    ...
    pub fn get_total_hdr_size(&self) -> usize {
        let transport_hdr_size = match self.payload.header {
            ...
            // FLUX-TODO addr=0xdb6e flavor=explicit_panic
    ...
    pub fn set_transport_checksum(&mut self) {
        ...
        match self.payload.header {
            ...
            _ => {
                // FLUX-TODO addr=0x19e94 flavor=explicit_panic
                flux_support::assert(false);
                unimplemented!();
            }
        }
    ...
    pub fn encode(&self, buf: &mut [u8]) -> SResult<usize> {
        ...
        // FLUX-OPT addr=0xdb10 flavor=unwrap_option
        flux_support::assert(done.is_some());
