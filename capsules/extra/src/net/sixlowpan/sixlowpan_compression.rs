...
pub trait ContextStore {
    ...
    fn get_context_0(&self) -> Context {
        match self.get_context_from_id(0) {
            Some(ctx) => ctx,
            None => {
                // FLUX-OPT addr=0x18efc flavor=explicit_panic
...
fn decompress_ext_hdr(
    ...
    *consumed += 1;
    ...
    // FLUX-TODO addr=0xd0be flavor=slice_order
...
// FLUX-TODO-FN-LEVEL reason=mass-refactor-fn-entry addrs=[0xd0de, 0xd0e8, 0xd0fc, 0xd126, 0xd14e, 0xd15e, 0xd18e, 0xd19c, 0xd1b2, 0xd1be, 0xd1d4] flavor=mixed
pub fn decompress(
    ctx_store: &dyn ContextStore,
    buf: &[u8],
    src_mac_addr: MacAddress,
...
fn decompress_tf(ip6_header: &mut IP6Header, iphc_header: u8, buf: &[u8], consumed: &mut usize) {
    ...
    if fl_compressed {
        ...
        // FLUX-TODO addr=0xd194 flavor=bounds
        flux_support::assert(*consumed + 1 < buf.len());
        // FLUX-TODO addr=0xd1a8 flavor=bounds
    ...
    if uses_context {
        match dam_mode {
            iphc::DAM_INLINE => {
                ...
                // FLUX-TODO addr=0xd11c flavor=slice_order
...
fn decompress_iid_link_local(
    ...
    match mode {
        ...
        iphc::SAM_MODE1 | iphc::DAM_MODE1 => {
            ...
            // FLUX-TODO addr=0xd486 flavor=slice_order
        ...
        iphc::SAM_MODE2 | iphc::DAM_MODE2 => {
            ...
            flux_support::assume(dst.len() == 2);
            // FLUX-TODO addr=0xd480 flavor=slice_order
            flux_support::assert(*consumed + 2 <= buf.len());
        ...
        _ => {
            // FLUX-TODO addr=0xd4a6 flavor=explicit_panic
            flux_support::assert(false);
            panic!("Unreachable case")
        },
    }
...
// FLUX-TODO-FN-LEVEL addrs=[0xd6a6] flavor=slice_end
fn decompress_iid_context(
    ...
    match mode {
        ...
        iphc::SAM_MODE1 | iphc::DAM_MODE1 => {
            ...
            // FLUX-TODO addr=0xd698 flavor=slice_order
            flux_support::assert(*consumed + 8 <= buf.len());
            dst.copy_from_slice(&buf[*consumed..*consumed + 8]);
            *consumed += 8;
        }
        ...
        iphc::SAM_MODE2 | iphc::DAM_MODE2 => {
            ...
            // FLUX-TODO addr=0xd692 flavor=slice_order
            flux_support::assert(*consumed + 2 <= buf.len());
        ...
        // FLUX-TODO addr=0xd6c0 flavor=explicit_panic
...
fn decompress_udp_ports(udp_nhc: u8, buf: &[u8], consumed: &mut usize) -> (u16, u16) {
    ...
    if src_compressed && dst_compressed {
        ...
        *consumed += 3;
        ...
        flux_support::assert(*consumed + 2 <= buf.len());
        // FLUX-TODO addr=0xd13a flavor=slice_order
        flux_support::assert(*consumed <= *consumed + 2);
        ...
        *consumed += 3;
        ...
        // FLUX-TODO addr=0xd134 flavor=slice_order
        flux_support::assert(*consumed + 2 <= buf.len());
        src_port = u16::from_be(network_slice_to_u16(&buf[*consumed..*consumed + 2]));
...
fn decompress_udp_checksum(
    ...
    if (udp_nhc & nhc::UDP_CHECKSUM_FLAG) != 0 && !is_fragment {
        ...
        // FLUX-TODO addr=0xd112 flavor=slice_order
        flux_support::assert(*consumed + 2 <= buf.len());
