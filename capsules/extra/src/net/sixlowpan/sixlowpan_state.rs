...
fn set_frag_hdr(
    ...
    if !is_frag1 {
        // FLUX-TODO addr=0xdb5a flavor=bounds
        flux_support::assert(4 < hdr.len());
...
impl<'a> TxState<'a> {
    ...
    // FLUX-TODO-FN-LEVEL addrs=[0x19a5c] flavor=bounds
    fn prepare_first_fragment<'b>(
        ...
        if written <= remaining_capacity {
            ...
            // FLUX-TODO addr=0x19a0a flavor=slice_end
        ...
        // FLUX-TODO addr=0x19a14 flavor=slice_end
    ...
    fn prepare_next_fragment<'b>(
        ...
        if payload_len > 0 {
            ...
            // FLUX-TODO addr=0x19a1a flavor=slice_order
...
impl<'a> RxState<'a> {
    ...
    fn receive_next_frame(
        ...
        let uncompressed_len = if dgram_offset == 0 {
            // FLUX-TODO addr=0x1e1cc flavor=slice_end
            ...
            // FLUX-TODO addr=0x1e1ba flavor=slice_end
    ...
    fn end_receive(
        ...
        client.map(move |client| {
            ...
            // Notes: blocked-cell
            // FLUX-TODO addr=0xa65e flavor=unwrap_option
...
fn slice_view(buf: &[u8], off: usize, len: usize) -> &[u8] {
    // FLUX-TODO addr=0x1e198 flavor=slice_order
...
impl<'a, A: time::Alarm<'a>, C: ContextStore> Sixlowpan<'a, A, C> {
    ...
    fn receive_frame(
        ...
        if is_fragment(packet) {
            // FLUX-TODO addr=0x1e1aa flavor=slice_end
    ...
    fn receive_single_packet(
        ...
        rx_state.map_or((None, Err(ErrorCode::NOMEM)), |state| {
            ...
            // FLUX-TODO addr=0x1e1b0 flavor=unwrap_option
            ...
            match decompressed {
                Ok((consumed, written)) => {
                    let remaining = payload_len - consumed;
                    // FLUX-TODO addr=0x1e1c4 flavor=slice_order
    ...
    // FLUX-TODO-FN-LEVEL addrs=[0x1e1ee] flavor=bounds
    fn receive_fragment(
        &self,
        frag_payload: &[u8],
        payload_len: usize,
