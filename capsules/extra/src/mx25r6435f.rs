    ...
    > MX25R6435F<'a, S, P, A>
{
    ...
    fn enable_write(&self) -> Result<(), ErrorCode> {
        ...
        self.txbuffer
            .take()
            .map_or(Err(ErrorCode::RESERVE), |txbuffer| {
                // FLUX-TODO addr=0x166cc flavor=bounds
                    ...
                    .map_or(Err(ErrorCode::RESERVE), |txbuffer| {
                        self.rxbuffer
                            ...
                            .map_or(Err(ErrorCode::RESERVE), move |rxbuffer| {
                                ...
                                // Notes: blocked-cell
                                // FLUX-TODO addr=0x16556 flavor=bounds
                                ...
                                if let Err((err, txbuffer, rxbuffer)) = self.spi.read_write_bytes(
                                    ...
                                    // Notes: blocked-cell
                                    // FLUX-TODO addr=0x1654e flavor=unwrap_option
...
{
    // FLUX-TODO-FN-LEVEL reason=multi-candidate-fn-entry addrs=[0x1e71c, 0x1e72a, 0x1e77a] flavor=bounds
    ...
    fn alarm(&self) {
        ...
        self.txbuffer.take().map(|write_buffer| {
            self.rxbuffer.take().map(move |read_buffer| {
                // FLUX-TODO addr=0x1ebae flavor=bounds
