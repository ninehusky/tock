    ...
    > MX25R6435F<'a, S, P, A>
{
    ...
    fn enable_write(&self) -> Result<(), ErrorCode> {
        ...
        self.txbuffer
            .take()
            .map_or(Err(ErrorCode::RESERVE), |txbuffer| {
                // FLUX-TODO addr=0x16994 flavor=bounds
                    ...
                    .map_or(Err(ErrorCode::RESERVE), |txbuffer| {
                        self.rxbuffer
                            ...
                            .map_or(Err(ErrorCode::RESERVE), move |rxbuffer| {
                                ...
                                // Notes: blocked-cell
                                // FLUX-TODO addr=0x1681e flavor=bounds
                                ...
                                if let Err((err, txbuffer, rxbuffer)) = self.spi.read_write_bytes(
                                    ...
                                    // Notes: blocked-cell
                                    // FLUX-TODO addr=0x16816 flavor=unwrap_option
...
{
    // FLUX-TODO-FN-LEVEL reason=multi-candidate-fn-entry addrs=[0x1f2e4, 0x1f2f2, 0x1f342] flavor=bounds
    ...
    fn alarm(&self) {
        ...
        self.txbuffer.take().map(|write_buffer| {
            self.rxbuffer.take().map(move |read_buffer| {
                // FLUX-TODO addr=0x1f5be flavor=bounds
