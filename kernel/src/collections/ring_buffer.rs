...
impl<'a, T: Copy> RingBuffer<'a, T> {
    ...
    pub fn as_slices(&'a self) -> (Option<&'a [T]>, Option<&'a [T]>) {
        if self.head < self.tail {
            // FLUX-OPT addr=0x1371e line=56 flavor=slice_end
            ...
            // in-boundsness.
            // FLUX-OPT addr=0x13732 line=60 flavor=explicit_panic
...
impl<T: Copy> queue::Queue<T> for RingBuffer<'_, T> {
    ...
    fn has_elements(&self) -> bool {
        // FLUX-OPT line=83 flavor=rem_by_zero addrs=[
        //     0x7ecc, 0x1064a,
        // ]
    ...
    fn enqueue(&mut self, val: T) -> bool {
        if self.is_full() {
            ...
            // FLUX-OPT addr=0x10652 line=113 flavor=bounds
            flux_support::assert(self.tail < self.ring.len());
    ...
    fn dequeue(&mut self) -> Option<T> {
        if self.has_elements() {
            // FLUX-OPT line=153 flavor=bounds addrs=[
            //     0x10aac, 0x2c26,
            // ]
            flux_support::assert(self.head < self.ring.len());
            let val = self.ring[self.head];
