...
impl<'a, T> SubSliceMut<'a, T> {
    ...
    fn active_slice(&self) -> &[T] {
        // FLUX-TODO addr=0xa100 flavor=slice_order
    ...
    I: SliceIndex<[T]>,
{
    ...
    fn index(&self, idx: I) -> &Self::Output {
        // FLUX-TODO addrs=[0x14ff6, 0x19e80, 0x1f99c] flavor=slice_order
        // FLUX-TODO addrs=[0x15000, 0x19e8a, 0x1f9a6, 0x1f9b0] flavor=slice_end
        // FLUX-TODO addr=0x19ea6 flavor=bounds
        flux_support::assert(self.active_range.start <= self.active_range.end && self.active_range.end <= self.internal.len());
